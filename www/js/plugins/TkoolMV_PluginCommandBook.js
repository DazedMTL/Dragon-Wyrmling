...
(function () {
    ...
    Game_Interpreter.prototype.pluginCommandBook_指定位置にアニメーション表示 = function (args) {
        ...
        var wait = args[3] && (args[3] === 'ウェイトあり' || args[3].toLowerCase() === 'wait');
        var mirror = [args[3], args[4]].includes('反転') || [args[3], args[4]].includes('mirror');
        $gameScreen.startAnimation(x, y, id, false, mirror);
    ...
    Game_Interpreter.prototype.pluginCommandBook_指定位置にループアニメーション表示 = function (args) {
        ...
        var y = parseIntStrict(args[1]);
        var id = parseIntStrict(args[2]);
        var mirror = ['反転', 'mirror'].includes(args[3]);
        $gameScreen.startAnimation(x, y, id, true, mirror);
    };
    ...
    Object.defineProperty(Game_Screen.prototype, 'animationMirror', {
        get: function () {
            return !!this._animationMirror;
        },
        configurable: false
    });
    ...
    Spriteset_Base.prototype.createAnimationContainer = function () {
        ...
        this._fieldAnimationSprite = new Sprite_Base();
        this._fieldAnimationSprite.setFrame(0, 0, this.width, this.height);
        this.addChild(this._fieldAnimationSprite);
    };
    ...
    Spriteset_Base.prototype.updateAnimationContainer = function () {
        ...
        if (id > 0 && id < $dataAnimations.length) {
            if ($gameScreen._animationLoop) {
                this._animationContainer.x = $gameScreen.animationContainerX;
                this._animationContainer.y = $gameScreen.animationContainerY;
                this._animationContainer.startAnimation($dataAnimations[id], $gameScreen.animationMirror, 0);
                this._animationId = id;
            } else {
                let sprite = new Sprite_AnimationWrapper();
                sprite.x = $gameScreen.animationContainerX;
                sprite.y = $gameScreen.animationContainerY;
                this._fieldAnimationSprite.addChild(sprite);
                sprite.startAnimation($dataAnimations[id], $gameScreen.animationMirror, 0);
            }
            $gameScreen.clearAnimation();
        }
        if (!this._animationContainer.isAnimationPlaying() && this._animationId > 0) {
            if ($gameScreen.animationLoop) {
                this._animationContainer.startAnimation($dataAnimations[this._animationId], $gameScreen.animationMirror, 0);
            } else {
                this._animationId = 0;
            }
        }
    ...
    class Sprite_AnimationWrapper extends Sprite_Base {
        constructor() {
            super();
        }

...
Window_Base.prototype.convertExtraEscapeCharacters = function(text) {
    ...
    text = text.replace(/\x1bISD\[(\d+)\]/gi, function() {
        return this.escapeIconItem(arguments[1], $dataSkills, 1);
    }.bind(this));
    // \IT[n]
    ...
    // ’jŽÝ’Ç‰Á
    // \WHP[n]
    text = text.replace(/\x1bWHP\[(\d+)\]/gi, function() {
        return $dataWeapons[parseInt(arguments[1])].params[0];
    }.bind(this));
    // \WAT[n]
    text = text.replace(/\x1bWAT\[(\d+)\]/gi, function() {
        return $dataWeapons[parseInt(arguments[1])].params[2];
    }.bind(this));
    // Finish
    return text;
};
...
Window_Base.prototype.escapeIconItem = function(n, database,d) {
  if (d == 1){
    return '\x1bI[' + database[n].iconIndex + ']' + database[n].name + " : " + database[n].description;
  } else {
